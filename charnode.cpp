...
CharNode *CharNode::hentNeste() const { return m_neste; }

void CharNode::settNeste(CharNode *c) { m_neste = c; }
void CharNode::skrivBaklengs() const {
  if (m_neste)
    m_neste->skrivBaklengs(); // Main: liste->skrivBaklengs();
  std::cout << m_tegn;
}
int CharNode::hentAntall() { return s_antall; }

CharNode::~CharNode() { s_antall--; }

char CharNode::hentData() const { return m_tegn; }
} // namespace ADS101
