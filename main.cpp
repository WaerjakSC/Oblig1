...
template<typename T>
void printStack2(std::stack<T> &liste);
 ...
 * Since std::stack doesn't seem to have its own operator overload to print out its contents, I added a small function to do that (stackReadOut())
 */
template<typename T>
void printStack2(std::stack<T> &liste)
{
    ...
    std::cout << "\nCurrent contents of stack: ";
    stackReadOut(liste);
    while (!liste.empty()) // sidenote - it seems I originally misunderstood the purpose of stack's empty function,
                           // so in my own versions empty() actually clears the stack instead of returning a bool
    {
        liste.pop();
    }
    std::cout << "\nEmptied stack... ";
